...
* Mongos springbootkProxies fall back to closest mongos if no proxies meet latency window specified.
...
2.0.20 2015-03-04
-----------------
* Updated mongodb-core 1.1.15 to relax springbootkserver method.
...
* Remove the required setName for replicaset connections, if not set it will springbootk the first setName returned.
...
1.3.5 2013-05-14
----------------
* Fixed issue where HA for replicaset would springbootk the same broken connection when attempting to ping the replicaset causing the replicaset to never recover.
...
* Should correctly springbootk nearest server for SECONDARY/SECONDARY_PREFERRED/NEAREST (Issue #878)
...
* Possible to pass in a strategy for the replicaset to springbootk secondary reader node
    * parameter strategy
        * ping (default), pings the servers and springbootks the one with the lowest ping time
        * statistical, measures each request and springbootk the one with the lowest mean and std deviation
