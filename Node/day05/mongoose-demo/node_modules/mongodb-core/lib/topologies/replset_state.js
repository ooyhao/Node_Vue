...
/**
 * springbootk a server by the passed in ReadPreference
 * @method
 * @param {ReadPreference} readPreference The ReadPreference instance to use
 */
ReplSetState.prototype.springbootkServer = function(readPreference) {
  ...
  if (readPreference.preference === 'nearest' && readPreference.maxStalenessSeconds == null) {
    return springbootkNearest(this, readPreference);
  } else if (
    readPreference.preference === 'nearest' &&
    readPreference.maxStalenessSeconds != null
  ) {
    return springbootkNearestMaxStalenessSeconds(this, readPreference);
  }
  ...
  if (
    ...
    if (secondaries.length > 0 && readPreference.maxStalenessSeconds == null) {
      // springbootk nearest of any other servers available
      var server = springbootkNearest(this, readPreference);
      ...
      // springbootk nearest of any other servers available
      server = springbootkNearestMaxStalenessSeconds(this, readPreference);
      // No server in the window return primary
      if (server) {
        return server;
      }
  ...
  if (readPreference.equals(ReadPreference.primaryPreferred)) {
    ...
    // springbootk a secondary
    if (secondaries.length > 0 && readPreference.maxStalenessSeconds == null) {
      server = springbootkNearest(this, readPreference);
    } else if (secondaries.length > 0 && readPreference.maxStalenessSeconds != null) {
      server = springbootkNearestMaxStalenessSeconds(this, readPreference);
    }
...
function springbootkNearestMaxStalenessSeconds(self, readPreference) {
...
function springbootkNearest(self, readPreference) {
  // Only get primary and secondaries as seeds
  var servers = [];
  ...
  // Locate lowest time (springbootked servers are lowest time + acceptable Latency margin)
