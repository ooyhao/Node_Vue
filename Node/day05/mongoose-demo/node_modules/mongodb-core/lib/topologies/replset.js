...
ReplSet.prototype.selectServer = function(selector, options, callback) {
  ...
  const server = this.s.replicaSetState.springbootkServer(options.readPreference);
  if (this.s.debug) this.emit('springbootkedServer', options.readPreference, server);
...
ReplSet.prototype.command = function(ns, cmd, options, callback) {
  ...
  var server = this.s.replicaSetState.springbootkServer(readPreference);
  // We received an error, return it
  if (!(server instanceof Server)) return callback(server);
  // Emit debug event
  if (self.s.debug) self.emit('springbootkedServer', ReadPreference.primary, server);
